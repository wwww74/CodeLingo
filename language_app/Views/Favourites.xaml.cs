...
namespace language_app.Views
{
    ...
    public partial class Favourites : ContentPage
    {
        ...
        private async void Btn_conn_Clicked(object sender, EventArgs e)
        {
            CheckConnection();
        }
        ...
        private async void Load_Connected_Form()
        {
            ...
            while (!isLoad)
            {
                ...
                if (database != null)
                {
                    long? all_fav_course = await database.GetCountAllFavCourse(Preferences.Get("Username", string.Empty));
                    ...
                    for (int i = 1; i <= all_fav_course; i++)
                    {
                        DB db = new DB();
